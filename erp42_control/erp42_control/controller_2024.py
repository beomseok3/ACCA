...
class PathHandler:
    def __init__(self, node, path_topic,parking):
        qos_profile = QoSProfile(depth=10)
        node.create_subscription(Path, path_topic, self.callback_path, qos_profile)
        node.create_subscription(Path, parking, self.callback_path, qos_profile)
        self.cx = []
        self.cy = []
        self.cyaw = []
...
class Drive:
    def __init__(self, node, state, path):
        ...
        node.create_subscription(
            Int32,"estop",self.callback_estop,qos_profile
        )
...
def main(args=None):
    ...
    path_tracking = PathHandler(node, "/path1","parking_path") #path1
