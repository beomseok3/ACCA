...
class Detection:
        ...
        if self.state == PARKING_STATE.SEARCH:
            for p1 in [(pose.position.x, pose.position.y) for pose in msg.poses]:
                rotated_p1 = self.rotate_points(
                    np.array([p1]),
                    CONTROL_PARKING.standard_point.yaw,
                    np.array([CONTROL_PARKING.standard_point.x, CONTROL_PARKING.standard_point.y]),
                )
    ...
    def in_detection_area(self, point):
        ...
        if CONTROL_PARKING.min_x <= point[0] <= CONTROL_PARKING.max_x:
            if CONTROL_PARKING.min_y <= point[1] <= CONTROL_PARKING.max_y:
                return True
        return False
...
class CONTROL_PARKING():
    ...
    def update_parking_path_if_needed(self):
        ...
        for i in range(len(Detection.low_y_cone) - 1):
            # print(self.low_y_cone)
            dist = Detection.low_y_cone[i + 1][0] - Detection.low_y_cone[i][0]
            print(dist)
            if dist > 4.0:
                self.idx = i
    ...
    def adjust_parking_path(self):
        ...
        goal_pose_c = Detection.rotate_points(
            np.array(
                [Detection.low_y_cone[self.idx][0] + 1, Detection.low_y_cone[self.idx][1] - 1.5]
            ),
            -self.standard_point.yaw,
            np.array([self.standard_point.x, self.standard_point.y]),
        )
        ...
        self.parking_path[:, :2] = Detection.rotate_points(
    ...
    def control_parking(self):
        ...
        if self.target_idx >= len(self.path_cx) - self.goal - 1:
            ...
            if self.parking_state == PARKING_STATE.SEARCH:
                self.parking_state = PARKING_STATE(self.state.value + 1)  # SEARCH -> PARKING
                self.goal = 0
                ...
                self.parking_state = PARKING_STATE(self.state.value + 1)  # PARKING -> STOP
            ...
            if self.parking_state == PARKING_STATE.STOP:
                if time.time() - self.stop_start_time >= 5.0:
                    self.parking_state = PARKING_STATE(self.state.value + 1)
