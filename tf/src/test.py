...
class TestNode(Node):
    def __init__(self):
        ...
        self.tf_buffer = Buffer(Duration(seconds=0.1), self)
    ...
    def callback(self, msg):
        ...
        for pose in msg.poses:
            ...
            if self.tf_buffer.can_transform("map", "velodyne", Time.from_msg(tf_pose.header.stamp), Duration(seconds=0.1)):
                transformed_pose = self.tf_buffer.transform(tf_pose, "map", Duration(seconds=0.1))
                new_pose = Pose()
