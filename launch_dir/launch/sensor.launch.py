...
def generate_launch_description():
    ...
    erp_launch = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            os.path.join(pkg_path_erp, "launch", "erp42.launch.py")
        )
    )

    ntrip_launch = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            os.path.join(pkg_path_ntrip, "launch", "ntrip_client_RTS.launch.py")
        )
    )

    gps_launch = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            os.path.join(pkg_path_ublox, "launch", "ublox_gps_node-launch.py")
        )
    )

    imu_launch = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            os.path.join(pkg_path_xsens, "launch", "xsens_mti_node.launch.py")
        )
    )
    ...
    lidar_launch = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            os.path.join(
                pkg_path_lidar, "launch", "velodyne-all-nodes-VLP32C-launch.py"
            )
        )
    )
    ...
    camera_launch = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            os.path.join(pkg_path_camera, "launch", "camera.launch.py")
        )
    )
